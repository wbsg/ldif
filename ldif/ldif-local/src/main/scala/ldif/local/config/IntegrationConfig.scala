...
case class IntegrationConfig(sources : Traversable[String], linkSpecDir : File, mappingFile : File, outputFile : File,  properties : Properties, runSchedule : String) {
...
object IntegrationConfig
{
  ...
  def load(configFile : File) =
  {
    ...
    IntegrationConfig(
      sources = sourceSet,//TODO use File instead of String .map(new File(_)),
