...
object Main
{
  ...
  private var ebType = EntityBuilderType.InMemory
  ...
  private def executeMappingPhase(config: LdifConfiguration, quadReaders: scala.Seq[QuadReader]): QuadReader = {
  ...
  def executeLinkingPhase(config: LdifConfiguration, r2rReader: QuadReader): QuadReader = {
  ...
  private def executeURITranslation(clonedR2rReader: QuadQueue, linkReader: QuadReader): QuadReader = {
  ...
  def runIntegrationFlow(configFile: File, debugMode: Boolean) {
    ...
    val integratedReader: QuadReader = executeURITranslation(clonedR2rReader, linkReader)
  ...
  def setupParameters() {
    // Setup EntityBuilder
    if(configProperties.getPropertyValue("entityBuilderType", "in-memory").toLowerCase=="in-memory")
      ebType = EntityBuilderType.InMemory
    else if(configProperties.getPropertyValue("entityBuilderType", "in-memory").toLowerCase=="voldemort")
      ebType = EntityBuilderType.Voldemort
    else {
      log.severe("No Entity Builder of type " + configProperties.getPropertyValue("entityBuilderType") + " available.")
      System.exit(1)
    }
