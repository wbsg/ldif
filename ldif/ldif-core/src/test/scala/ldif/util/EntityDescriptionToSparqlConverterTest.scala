...
class EntityDescriptionToSparqlConverterTest extends FlatSpec with ShouldMatchers {
  ...
  it should "convert an EntityDescription to a SPARQL query" in {
    ...
    // use default graph
    EntityDescriptionToSparqlConverter.convert(ed, true).head.toString should equal ("(SELECT DISTINCT ?ldifvar1 ?ldifvar2 ?SUBJ  WHERE  {  { ?SUBJ <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <Class> . ?SUBJ <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <Another_Class> .  } ?SUBJ <p1> ?ldifph3 . ?ldifph3 <p2> ?ldifvar1 . ?SUBJ <p1> ?ldifph5 . ?ldifph5 <p3> ?ldifvar2 .  } ORDER BY ?SUBJ,ArrayBuffer(ldifph1, ldifph2, ldifph4, ldifph6))")

    // use specific named graph
    EntityDescriptionToSparqlConverter.convert(ed, "http://myGraph" ).head.toString should equal ("(SELECT DISTINCT ?ldifvar1 ?ldifvar2 ?SUBJ  FROM <http://myGraph>  WHERE  {  { ?SUBJ <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <Class> . ?SUBJ <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <Another_Class> .  } ?SUBJ <p1> ?ldifph3 . ?ldifph3 <p2> ?ldifvar1 . ?SUBJ <p1> ?ldifph5 . ?ldifph5 <p3> ?ldifvar2 .  } ORDER BY ?SUBJ,ArrayBuffer(ldifph1, ldifph2, ldifph4, ldifph6))")
