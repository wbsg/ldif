...
class EntityDescriptionToSparqlConverterTest extends FlatSpec with ShouldMatchers {
  ...
  it should "convert an EntityDescription to a SPARQL query" in {
    ...
    EntityDescriptionToSparqlConverter.convert(ed).head.toString should equal ("(SELECT ?ldifvar1 ?ldifvar1graph ?ldifvar2 ?ldifvar2graph ?SUBJ ?ldifph1  {  { GRAPH ?ldifph1 { ?SUBJ <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <Class> .  } GRAPH ?ldifph2 { ?SUBJ <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <Another_Class> .  }  } GRAPH ?ldifph4 { ?SUBJ <p1> ?ldifph3 .  } GRAPH ?ldifvar1graph { ?ldifph3 <p2> ?ldifvar1 .  } GRAPH ?ldifph6 { ?SUBJ <p1> ?ldifph5 .  } GRAPH ?ldifvar2graph { ?ldifph5 <p3> ?ldifvar2 .  }  } ORDER BY ?SUBJ,ldifph1)")
  }
